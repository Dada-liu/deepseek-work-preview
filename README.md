...
1. **预装插件**：`seed_preinstalled_bundles` 在拉起 DSH 前，把预装插件（`dshmarket`、`dsh-notifier-plugin`、`dsh-work-upgrade`、`dsh-prompt-history-plugin`、`dsh-wooden-fish`、`dsh-usage`）写入 web profile 的 `dsh.profile.bundles`（全新 profile 直接写入；用户自定义过的清单只追加缺失的预装项）
