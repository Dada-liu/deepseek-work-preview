...
1. **Preinstall the plugins**: `seed_preinstalled_bundles` writes the preinstalled plugins (`dshmarket`, `dsh-notifier-plugin`, `dsh-version-plugin`, `dsh-prompt-history-plugin`, `dsh-wooden-fish`) into the web profile's `dsh.profile.bundles` before starting DSH (written directly on a fresh profile; a user-customized list only gets the missing preinstalled entries appended)
